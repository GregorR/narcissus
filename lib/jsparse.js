...
Narcissus.parser = (function() {
    ...
    const blackLists = { 160: {}, 185: {}, harmony: {}, harmonyWClasses: {} };
    ...
    blackLists[185][CLASS] = true;
    blackLists.harmony[CLASS] = true;
    blackLists.harmony[WITH] = true;

    blackLists.harmonyWClasses[WITH] = true;
    ...
    function Statement(t, x) {
        ...
        switch (tt) {
          ...
          case FUNCTION:
            ...
            return FunctionDefinition(t, x, true, true, x.topLevel ? DECLARED_FORM : STATEMENT_FORM, comments);

          case CLASS:
            return ClassDeclaration(t, x);
    ...
    /*
     * FunctionDefinition :: (tokenizer, compiler context, boolean, boolean,
     *                        DECLARED_FORM or EXPRESSED_FORM or STATEMENT_FORM,
     *                        [string] or null or undefined)
     *                    -> node
     */
    function FunctionDefinition(t, x, requireType, requireName, functionForm, comments) {
        var tt;
        var f = new Node(t, { params: [], paramComments: [], paramAnnotations: [] });
            ...
            do {
                ...
                switch (tt) {
                  ...
                  case IDENTIFIER:
                    ...
                    f.paramAnnotations.push(Annotations(t));
                    break;
                  default:
    ...
    /*
     * ClassDeclaration :: (tokenizer, compiler context) -> node
     */
    function ClassDeclaration(t, x) {
        var tt;
        var f = new Node(t, { params: [], funDecls: [], varDecls: [] });
        if (t.match(IDENTIFIER))
            f.name = t.token.value;
        else
            throw t.newSyntaxError("missing class identifier");
        f.annotation = Annotations(t);

        var inModule = x ? x.inModule : false;
        var x2 = new StaticContext(f, null, inModule, true);

        // get any extends clause
        f.extends = null;
        if (t.match(IDENTIFIER)) {
            if (t.token.value === "extends") {
                t.mustMatch(IDENTIFIER);
                f.extends = t.token.value;
                f.extendsAnnotation = Annotations(t);
            } else {
                throw t.newSyntaxError("illegal identifier");
            }
        }

        t.mustMatch(LEFT_CURLY);
        f.children = ClassBody(t, x2);
        t.mustMatch(RIGHT_CURLY);

        f.end = t.token.end;
        x.parentScript.classDecls.push(f);
        return f;
    }

    /*
     * ClassBody :: (tokenizer, compiler context) -> [node]
     */
    function ClassBody(t, x) {
        var tt;
        var n = [];

        try {
            while (!t.done && t.peek(true) !== RIGHT_CURLY)
                n.push(ClassElement(t, x));
        } catch (e) {
            if (t.done)
                t.unexpectedEOF = true;
            throw e;
    ...
    /*
     * ClassElement :: (tokenizer, compiler context) -> node
     */
    function ClassElement(t, x) {
        var tt;
        var n = null;
        ...
        if (tt === "constructor") {
            ...
            n = ClassPropertyDefinition(t, x);

        } else {
            n = PrototypePropertyDefinition(t, x);
    ...
    /*
     * PrototypePropertyDefinition :: (tokenizer, compiler context) -> node
     */
    function PrototypePropertyDefinition(t, x) {
        var tt;

        t.mustMatch(IDENTIFIER);

        var n = new Node(t, { type: INSTANCE_PROPERTY });

        if (n.value === "public") {
            n.public = true;
        } else if (n.value === "private") {
            n.public = false;
        } else {
            t.unget();
            n.public = true;
        ...
        n.definition = ExportableDefinition(t, x);

        return n;
    }

    /*
     * Constructor :: (tokenizer, compiler context) -> node
     */
    function Constructor(t, x) {
        var tt;
        t.mustMatch(IDENTIFIER);
        var f = new Node(t, { type: CONSTRUCTOR, params: [], paramAnnotations: [] });
        f.name = t.token.value;
        f.annotation = Annotations(t);
        ...
        t.mustMatch(LEFT_CURLY);
        f.children = ConstructorBody(t, x);
        t.mustMatch(RIGHT_CURLY);
    ...
    /*
     * ConstructorBody :: (tokenizer, compiler context) -> [node]
     */
    function ConstructorBody(t, x) {
        var tt;
        var n = [];

        try {
            while (!t.done && t.peek(true) !== RIGHT_CURLY)
                n.push(ConstructorElement(t, x));
        } catch (e) {
            if (t.done)
                t.unexpectedEOF = true;
            throw e;
    ...
    /*
     * ConstructorElement :: (tokenizer, compiler context) -> node
     */
    function ConstructorElement(t, x) {
        var tt;
        var n = null;
        ...
        if (t.match(IDENTIFIER)) {
            tt = t.token.value;
            t.unget();
            if (tt === "public" || tt === "private") {
                n = InstancePropertyDefinition(t, x);
    ...
    /*
     * InstancePropertyDefinition :: (tokenizer, compiler context) -> node
     */
    function InstancePropertyDefinition(t, x) {
        var tt;
    ...
    /*
     * ExportableDefinition :: (tokenizer, compiler context) -> node
     */
    function ExportableDefinition(t, x) {
        var tt;
        var n = null;
        var l;
        ...
        if (tt === "get" || tt === "set") {
            // getter or setter
            n = FunctionDefinition(t, x, true, true, DECLARED_FORM);

        } else {
            l = new Node(t);
            l.annotation = Annotations(t);
            ...
            } else if (t.match(LEFT_PAREN)) {
                t.unget();
                n = FunctionDefinition(t, x, false, false, EXPRESSED_FORM);
                // FIXME: really a declaration
                n.annotation = l.annotation;
                n.name = tt;

            } else {
                throw t.newSyntaxError("expected definition");
    ...
    function PrimaryExpression(t, x) {
        ...
        switch (tt) {
          case FUNCTION:
            n = FunctionDefinition(t, x, true, false, EXPRESSED_FORM);
            break;
            ...
            if (!t.match(RIGHT_CURLY)) {
                do {
                    ...
                    if ((t.token.value === "get" || t.token.value === "set") &&
                        ...
                        n.push(FunctionDefinition(t, x, true, true, EXPRESSED_FORM));
                    } else {
                        var comments = t.blockComments;
                        switch (tt) {
                          case IDENTIFIER: case NUMBER: case STRING:
                            id = new Node(t, { type: IDENTIFIER });
                            if (tt === IDENTIFIER) id.annotation = Annotations(t);
                            break;
                          case RIGHT_CURLY:
                            if (x.ecma3OnlyMode)
          ...
          case IDENTIFIER: case NUMBER: case STRING: case REGEXP:
            n = new Node(t);
            if (tt === IDENTIFIER) n.annotation = Annotations(t);
            break;
    ...
    /*
     * Annotation :: (tokenizer) -> null or array of tokens
     *
     * Parses an annotation if one is present
     */
    function Annotation(t) {
        if (t.peek() === ANNOTATION) {
            var ret = [];
            t.get();

            var tt = t.peek();
            if (tt === LEFT_BRACKET || tt === LEFT_CURLY || tt === LEFT_PAREN) {
                /* get a sequence until we lose our depth */
                t.get();
                ret.push(shallowDup(t.token));
                var depth = 1;
                while (depth > 0) {
                    var next = t.get();
                    ret.push(t.token);
                ...
                /* get a sequence until a depth-changer, comma, colon,
                 * annotation symbol or end of statement */
                var good = true;
                var depth = 0;
                while (good) {
                    var next = t.peek();
    ...
    /*
     * Annotations :: (tokenizer) -> array of annotations
     *
     * Parses any number of annotations
     */
    function Annotations(t) {
        var ret = [];
        var cur;
        while ((cur = Annotation(t)) !== null) {
            ret.push(cur);
        }
        return ret;
    }
